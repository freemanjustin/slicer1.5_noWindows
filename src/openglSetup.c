...
void setShaders(e *E) {
	...
	E->v = glCreateShader(GL_VERTEX_SHADER);
	E->f = glCreateShader(GL_FRAGMENT_SHADER);
	...
	glShaderSource(E->v, 1, &vv, NULL);
	glShaderSource(E->f, 1, &ff, NULL);
	...
	E->ground = glCreateProgram();
	glAttachShader(E->ground,E->v);
	glAttachShader(E->ground,E->f);

	glLinkProgram(E->ground);
	...
	E->atmo = glCreateProgram();
	glAttachShader(E->atmo,E->v);
	glAttachShader(E->atmo,E->f);

	glLinkProgram(E->atmo);
	...
	E->contrast = glCreateProgram();
	glAttachShader(E->contrast,E->v);
	glAttachShader(E->contrast,E->f);

	glLinkProgram(E->contrast);
	...
	E->texBlend = glCreateProgram();
	glAttachShader(E->texBlend,E->v);
	glAttachShader(E->texBlend,E->f);

	glLinkProgram(E->texBlend);
	...
	E->phong = glCreateProgram();
	glAttachShader(E->phong,E->v);
	glAttachShader(E->phong,E->f);
	
	glLinkProgram(E->phong);
}

void shaderEnable(GLuint p){
	glUseProgram(p);
}

void shaderDisable(GLuint p){
	glUseProgram(NULL);
}

void SetUniformParameter1i(GLuint p, const char *pszParameter, int n1){
	GLint	location;
	location = glGetUniformLocation(p, pszParameter); 
	//printf("%s location = %d, val = %d\n", pszParameter, (int)location, n1);
	glUniform1i(location, n1);
}

void SetUniformParameter1f(GLuint p, const char *pszParameter, float p1){
	GLint	location;
	location = glGetUniformLocation(p, pszParameter);
...
void SetUniformParameter3f(GLuint p, const char *pszParameter, float p1, float p2, float p3){
	GLint	location;
	location = glGetUniformLocation(p, pszParameter);
